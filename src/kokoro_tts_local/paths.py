...
def get_language_packs_dir() -> Path:
    """Directory containing optional, self-contained local TTS models."""
    configured = os.environ.get("YAW_YAW_LANGUAGE_PACKS_DIR")
    return Path(configured).expanduser().resolve() if configured else get_base_dir() / "language-packs"
