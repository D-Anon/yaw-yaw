...
MMS_LANGUAGES = frozenset(("tgl", "ceb", "ilo", "hil", "war"))
...
def _models_module():
    """Resolve lazily so engine tests and plugin-style backends remain isolated."""
    return importlib.import_module("kokoro_tts_local.models")
...
def engine_for(language=None, voice=None):
    """Resolve the backend centrally so callers never switch engines themselves."""
    if language in MMS_LANGUAGES or str(voice or "").startswith("mms_"):
        return "mms"
    return "kokoro"


def language_for_voice(voice):
    voice = str(voice or "")
    if voice.startswith("mms_") and voice[4:] in MMS_LANGUAGES:
        return voice[4:]
    models = _models_module()
    return models.get_language_code_from_voice(voice)
...
def synthesize(
    ...
    detected = language_for_voice(voice)
    ...
    for task_idx, (cur_voice, kind, val) in enumerate(tasks):
        ...
        if engine_for(language=language, voice=cur_voice) == "mms":
            from . import mms_engine
            generated = [(None, None, mms_engine.synthesize(
                speech_text, language_for_voice(cur_voice), speed=speed,
                device=device, cancelled=cancelled,
            ))]
            generator_context = nullcontext(iter(generated))
        else:
            ...
            generator_context = closing(pipeline.iter_speech(
                speech_text, voice=voice_payload, speed=speed, split_pattern=r'\n+'
            ))
