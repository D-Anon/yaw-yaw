...
MMS_LANGUAGE_NAMES = {code: name for name, code in LANGUAGES.items() if code in speech_service.MMS_LANGUAGES}
VOICES = [
...
def voices_for_language(language):
    if language in ("a", "b"):
        return [voice for voice in VOICES if voice.startswith(language)]
    if language in speech_service.MMS_LANGUAGES:
        return [f"mms_{language}"]
    return []


def engine_name(language):
    return "Meta MMS-TTS" if language in speech_service.MMS_LANGUAGES else "Kokoro"
...
class YawYawApp:
    def __init__(self, root):
        ...
        self.engine_info = tk.StringVar(value="Engine: Kokoro · Offline ready")
    ...
    def build_ui(self):
        ...
        ttk.Label(settings, text="LANGUAGE", style="Section.TLabel").pack(anchor="w", pady=(0, 2))
        ...
        ttk.Label(settings, textvariable=self.engine_info, style="Muted.TLabel",
                  font=("Segoe UI", 8, "bold")).pack(anchor="w", pady=(2, 0))
        self.language_packs_button = ttk.Button(
            settings, text="Manage language packs", style="Pill.TButton",
            command=self.show_language_packs,
        )
        self.language_packs_button.pack(anchor="w", pady=(4, 0))
    ...
    def update_voice_description(self, event=None):
        ...
        if voice.startswith("mms_"):
            language = voice[4:]
            name = MMS_LANGUAGE_NAMES.get(language, language.upper())
            self.voice_description.set(f"{name} · Local language voice")
            if hasattr(self, "voice_trait"):
                self.voice_trait.set("Natural single-speaker MMS model")
    ...
    def update_engine_info(self):
        if not hasattr(self, "engine_info"):
            return
        language = LANGUAGES.get(self.lang_var.get(), "a")
        if language in speech_service.MMS_LANGUAGES:
            from kokoro_tts_local import mms_engine
            state = "Offline ready" if mms_engine.pack_status(language)["ready"] else "Pack not installed"
        else:
            state = "Offline ready"
        self.engine_info.set(f"Engine: {engine_name(language)} · {state}")
    ...
    def change_language(self, event=None):
        ...
        is_kokoro = language not in speech_service.MMS_LANGUAGES
        if hasattr(self, "blend_enabled_var") and not is_kokoro:
            self.blend_enabled_var.set(False)
        if hasattr(self, "blend_toggle"):
            self.blend_toggle.configure(state="normal" if is_kokoro else "disabled")
        if hasattr(self, "blend_frame") and not is_kokoro:
            self.blend_frame.pack_forget()
        self.update_voice_description()
        self.update_blend_label()

    def show_language_packs(self):
        """Open the in-app installer for optional Philippine language models."""
        if self.pack_window is not None and self.pack_window.winfo_exists():
            self.pack_window.lift()
            self.pack_window.focus_force()
            self.refresh_language_pack_rows()
            return
        from kokoro_tts_local import mms_engine

        win = tk.Toplevel(self.root)
        self.pack_window = win
        win.title("Yaw-Yaw Language Packs")
        win.geometry("650x440")
        win.minsize(590, 400)
        win.transient(self.root)
        win.protocol("WM_DELETE_WINDOW", self.close_language_packs)

        outer = ttk.Frame(win, padding=18)
        outer.pack(fill="both", expand=True)
        ttk.Label(outer, text="Philippine Language Packs", style="Title.TLabel").pack(anchor="w")
        ttk.Label(
            outer,
            text="Install a language once, then use it completely offline. Yaw-Yaw switches engines automatically.",
        ...
        self.pack_rows = {}
        for row_number, status in enumerate(mms_engine.list_pack_statuses()):
            code = status["language"]
            row = ttk.Frame(table, style="Card.TFrame")
            row.pack(fill="x", pady=3)
            ttk.Label(row, text=status["name"], style="Card.TLabel", width=22,
                      font=("Segoe UI", 9, "bold")).pack(side="left")
            state_var = tk.StringVar()
            ttk.Label(row, textvariable=state_var, style="Muted.TLabel", width=22).pack(side="left", padx=(4, 8))
            button = ttk.Button(row, width=12, command=lambda lang=code: self.install_language_packs([lang]))
            button.pack(side="right")
            self.pack_rows[code] = (state_var, button)

        footer = ttk.Frame(outer)
        footer.pack(fill="x", pady=(10, 0))
        ttk.Label(
            footer, text="~145 MB each · Meta MMS · CC BY-NC 4.0 (non-commercial)",
            style="Muted.TLabel",
        ).pack(side="left")
        self.install_all_packs_button = ttk.Button(
            footer, text="Install all missing", command=self.install_all_missing_packs,
        )
        self.install_all_packs_button.pack(side="right")
        self.refresh_language_pack_rows()

    def close_language_packs(self):
        if self.pack_window is not None:
            try:
                self.pack_window.destroy()
            except tk.TclError:
                pass
        self.pack_window = None
        self.pack_rows = {}

    def refresh_language_pack_rows(self):
        from kokoro_tts_local import mms_engine
        for status in mms_engine.list_pack_statuses():
            row = self.pack_rows.get(status["language"])
            if not row:
                continue
            state_var, button = row
            if status["ready"]:
                size_mb = status["size_bytes"] / (1024 * 1024)
                state_var.set(f"Installed · {size_mb:.0f} MB")
                button.configure(text="Installed", state="disabled")
            else:
                state_var.set("Not installed · ~145 MB")
                button.configure(text="Install", state="disabled" if self.pack_installing else "normal")
        if hasattr(self, "install_all_packs_button") and self.install_all_packs_button.winfo_exists():
            missing = any(not item["ready"] for item in mms_engine.list_pack_statuses())
            self.install_all_packs_button.configure(
                state="normal" if (missing and not self.pack_installing) else "disabled"
            )
        self.update_engine_info()

    def install_all_missing_packs(self):
        from kokoro_tts_local import mms_engine
        missing = [item["language"] for item in mms_engine.list_pack_statuses() if not item["ready"]]
        if missing:
            self.install_language_packs(missing)

    def install_language_packs(self, languages):
        from kokoro_tts_local import mms_engine
        languages = [code for code in languages if code in mms_engine.LANGUAGE_PACKS]
        languages = [code for code in languages if not mms_engine.pack_status(code)["ready"]]
        if not languages or self.pack_installing:
            return False
        names = ", ".join(mms_engine.LANGUAGE_PACKS[code]["name"] for code in languages)
        estimated = 145 * len(languages)
        approved = messagebox.askyesno(
            "Install language packs",
            f"Download {names}?\n\nApproximate download: {estimated} MB\n"
            "License: CC BY-NC 4.0 (non-commercial)\n\n"
            "Internet is used only by the installer. Continue?",
            parent=self.pack_window or self.root,
        )
        if not approved:
            return False
        self.pack_installing = True
        self.refresh_language_pack_rows()
        self.status.set(f"Downloading language pack: {names}...")
        thread = threading.Thread(target=self._run_pack_installer, args=(languages,), daemon=True)
        thread.start()
        return True

    def _run_pack_installer(self, languages):
        """Run downloads out-of-process so this offline process stays offline."""
        from kokoro_tts_local import paths
        ...
        env.update({
            ...
            "KOKORO_BASE_DIR": str(paths.get_base_dir()),
        })
        command = [executable, "-u", "-m", "kokoro_tts_local.mms_engine", "install", *languages]
        flags = subprocess.CREATE_NO_WINDOW if os.name == "nt" else 0
        try:
            self.pack_process = subprocess.Popen(
                command, env=env, cwd=os.getcwd(), stdout=subprocess.PIPE,
                stderr=subprocess.STDOUT, text=True, encoding="utf-8", errors="replace",
                creationflags=flags,
            )
            last_line = ""
            for raw_line in self.pack_process.stdout:
                line = raw_line.strip()
                if line:
                    last_line = line
                    self.events.put(("pack_progress", line))
            return_code = self.pack_process.wait()
            if return_code:
                raise RuntimeError(last_line or f"Language installer exited with code {return_code}.")
            self.events.put(("pack_done", languages))
        except Exception as error:
            if not self.closing.is_set():
                self.events.put(("pack_error", error_message(error)))
        finally:
            self.pack_process = None
    ...
    def set_busy(self, busy):
        ...
        if hasattr(self, "blend_toggle"):
            current_language = LANGUAGES.get(self.lang_var.get(), "a")
            blend_available = current_language not in speech_service.MMS_LANGUAGES
            self.blend_toggle.configure(state="normal" if (not busy and blend_available) else "disabled")
        if hasattr(self, "secondary_voice_box"):
    ...
    def start_generation(self, autoplay=False):
        ...
        if language in speech_service.MMS_LANGUAGES:
            from kokoro_tts_local import mms_engine
            if not mms_engine.pack_status(language)["ready"]:
                open_manager = messagebox.askyesno(
                    "Language pack required",
                    f"{MMS_LANGUAGE_NAMES[language]} is not installed.\n\n"
                    "Would you like to open the Language Packs installer?",
                    parent=self.root,
                )
                if open_manager:
                    self.show_language_packs()
                return False
        speed = round(self.speed_var.get(), 1)
        try:
    ...
    def poll_events(self):
        ...
        for _ in range(100):
            ...
            if kind == "progress":
                ...
                self.pack_installing = False
                self.refresh_language_pack_rows()
                names = ", ".join(MMS_LANGUAGE_NAMES[code] for code in value)
                self.status.set(f"Language pack ready: {names}.")
            elif kind == "pack_error":
                self.pack_installing = False
                self.refresh_language_pack_rows()
                self.status.set("Language pack installation failed.")
                messagebox.showerror("Could not install language pack", value, parent=self.pack_window or self.root)
            elif kind == "done":
    ...
    def close(self):
        if self.closing.is_set():
            return
        self.closing.set()
        pack_process = getattr(self, "pack_process", None)
        if pack_process is not None and pack_process.poll() is None:
            try:
